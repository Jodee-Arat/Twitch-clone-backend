...
export const MESSAGES = {
  ...
  resetPassword: (token: string, metadata: SessionMetadata) =>
    ...
    `<b><a href="https://teastreamclone.ru/account/recovery/${token}">Сбросить пароль</a></b>\n\n` +
    ...
    `Спасибо за использование <b>TeaStreamClone</b>! 🚀`,
  deactivate: (token: string, metadata: SessionMetadata) =>
    ...
    `Спасибо, что пользуетесь <b>TeaStreamClone</b>! Мы всегда рады видеть вас на нашей платформе и надеемся, что вы останетесь с нами. 🚀\n\n` +
    `С уважением,\n` +
    `Команда TeaStreamClone`,
  accountDeleted:
    ...
    `<b><a href="https://teastreamclone.ru/account/create">Зарегистрироваться на Teastream</a></b>\n\n` +
  ...
  streamStart: (channel: User) =>
    ...
    `Смотрите здесь: <a href="https://teastreamclone.ru/${channel.username}">Перейти к трансляции</a>`,
  newFollowing: (follower: User, followersCount: number) =>
    `<b>У вас новый подписчик!</b>\n\nЭто пользователь <a href="https://teastreamclone.ru/${follower.username}">${follower.displayName}</a>\n\nИтоговое количество подписчиков на вашем канале: ${followersCount}`,
  newSponsorship: (plan: SponsorshipPlan, sponsor: User) =>
    `<b>🎉 Новое спонсор!</b>\n\n` +
    `Вы получили новое спонсорство на план <b>${plan.title}</b>.\n` +
    `💰 Сумма: <b>${plan.price} ₽</b>\n` +
    `👤 Спонсор: <a href="https://teastreamclone.ru/${sponsor.username}">${sponsor.displayName}</a>\n` +
    `📅 Дата оформления: <b>${new Date().toLocaleDateString()} в ${new Date().toLocaleTimeString()}</b>`,
  enableTwoFactor:
    `🔐 Обеспечьте свою безопасность!\n\n` +
    `Включите двухфакторную аутентификацию в <a href="https://teastreamclone.ru/dashboard/settings">настройках аккаунта</a>.`,
  verifyChannel:
    ...
    `Спасибо, что вы с нами и продолжаете развивать свой канал вместе с TeaStreamClone!`,
};
